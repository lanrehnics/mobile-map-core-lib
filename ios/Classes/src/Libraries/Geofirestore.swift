...
public class GFSQuery {
    internal class GFSQueryLocationInfo {
        ...
        var docSnaphot: DocumentSnapshot?
    }
        ...
        lockQueue.sync {
            ...
            if let docSnapshot = snapshot {
                if let l = snapshot?.get("l") as? [Double?], let lat = l[0], let lon = l[1] {
                    ...
                    updateLocationInfo(location, forKey: docSnapshot.documentID, snapShot: docSnapshot)
    ...
    internal func updateQueries() {
        ...
        for (offset: _, element: (key: key, value: info)) in self.locationInfos.enumerated(){
            if let location = info.location, let docSnapsot = info.docSnaphot{
                self.updateLocationInfo(location, forKey: key, snapShot: docSnapsot)
            }
        }
            ...
            case .documentEntered:
                ...
                geoFirestore.callbackQueue.async(execute: {
                    lockQueue.sync {
                        for (offset: _, element: (key: key, value: info)) in self.locationInfos.enumerated(){
                            if info.isInQuery != nil{
                                block(info.docSnaphot, info.location)
                            }
                        }
                    }
                })
